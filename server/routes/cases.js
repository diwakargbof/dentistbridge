...
// POST /api/cases — create case (dentist)
router.post('/', auth, async (req, res) => {
  const role = await getProfileRole(req.user.id);
  if (role !== 'dentist') return res.status(403).json({ error: 'Only dentists can create cases' });

  const { lab_id, service_id, patient_ref, notes, shade } = req.body;
  if (!lab_id || !service_id) return res.status(400).json({ error: 'lab_id and service_id are required' });

  const { data, error } = await supabase
    .from('cases')
    .insert({ lab_id, dentist_id: req.user.id, service_id, patient_ref, notes, shade })
    .select(CASE_SELECT)
    .single();
